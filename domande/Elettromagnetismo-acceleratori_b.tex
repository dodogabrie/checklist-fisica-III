\subsection[ Relazione differenziale tra tempo e tempo ritardato]{Dimostrare l’espressione
\[
...
.\] 
\subsection[ Definizione delle variabili $\hat{n}$, $\bs{\beta}$, $\bs{R}$, $\bs{r}$, $\bs{r}'$,  $t$, $t'$]{Date le definizioni ‘standard’ delle variabili $\hat{n}, \bs{\beta}, \bs{R}, \bs{r}, \bs{r}', t, t'$ , dimostrare le seguenti relazioni:
} \label{sec:3.b.2}
\begin{align*}
&1) \ \frac{\mbox{d} \bs{R}}{\mbox{d} t'} = -\bs{\beta}c 					&2)& \ \frac{\mbox{d}R}{\mbox{d}t'}=-\hat{n}\cdot\bs{\beta}c\\
& & &\\
&3) \ \frac{\mbox{d}\left(\bs{R}\cdot\bs{\beta}\right)}{\mbox{d}t'}=-\beta^2 c+\bs{R}\cdot\dot{\bs{\beta}}	&4)& \ \nabla R=\frac{\hat{n}}{\left(1-\hat{n}\bs{\beta}\right)}\\
& & &\\
...
\subsection[ Distribuzione in potenza per carica accelerata in moto non relativistico]{Calcolare la distribuzione in potenza in funzione dell’angolo di emissione per una carica accelerata in moto non relativistico.}
\label{sec:3.b.3}
...
\subsection[ Ricavare le leggi di trasformazione dei campi]{Ricavare esplicitamente le leggi di trasformazione di Lorentz del campo elettrico e del campo magnetico. Discutere, in particolare, il caso in cui, in un certo sistema di riferimento inerziale, il campo magnetico è nullo e il caso in cui il campo elettrico è nullo.}
\label{sec:3.b.4}
...
\begin{align*}
	F'^{01}=& -E'_{x}= \Lambda^{0}_{0}\Lambda^{1}_{0}F^{00} + \Lambda^{0}_{1}\Lambda^{1}_{1}F^{01} + 
		...
		\left( - \beta\gamma \right) \cdot \left( -\beta\gamma \right) F^{10} + 0 = \ldots = F^{01} = -E_{x}\\ 
		 & \\
...
\subsection[ Invarianti costruiti con il tensore dei campi]{Dire quali sono gli "invarianti di Lorentz" che si possono costruire con il tensore del campo elettromagnetico e ricavarne le espressioni esplicite in termini dei campi elettrico e magnetico. Ridiscutere, usando gli invarianti, il caso discusso nel punto precedente e discutere il caso in cui gli invarianti sono nulli.}
\label{sec:3.b.5}
...
\subsection[ Campi per una carica in moto fissato un sistema di riferimento]{Una carica elettrica Q si muove con velocità costante su una retta con velocità costante:
\begin{align*}
	 &x=Vt \\
	 &y=b \\ 
	 &z=0
...
\subsection[ Equazione del moto in forma covariante per una carica in un campo E.M. esterno]{Scrivere in forma covariante l'equazione del moto di una carica in un campo elettromagnetico esterno ("quadri-forza di Lorentz").}
\label{sec:3.b.7}
...
\subsection[ Reazione radiativa (già fatta in altra domanda)]{Dimostrare che la forza di reazione radiativa è: 
\[
	\bs{F}_{\text{rad}}=\frac{2q^2}{3c^3}\dot{\bs{a}}=z^2m_e \tau_e \dot{\bs{a}} \quad \quad \text{con } \tau_e=\frac{2r_{e}}{3c}=\left(6.2\cdot 10^{-24}\right)\text{ s}
.\]
...
\subsection[ Tensore energia impulso]{Dare la definizione del "tensore energia-impulso" del campo elettromagnetico e scrivere la sua relazione con la "densità di forza di Lorentz".}
\label{sec:3.b.9}
...
\subsection[\hspace{1mm} Teoremi di conservazione di energia ed impulso con campo E.M.]{Dire come si generalizzano i teoremi di conservazione dell'energia e dell'impulso a situazioni in cui sia presente un campo elettromagnetico.}
\label{sec:3.b.10}
...
\subsection[\hspace{1mm} Un esempio sul tensore degli sforzi]{Scrivere il tensore degli sforzi per un’onda e.m. piana che si propaga in una direzione definita come $\hat{n}$.}
\label{sec:3.b.11}
...
\[
	...
	\right)
.\] 
\subsection[\hspace{1mm} Un esempio sul tensore energia impulso]{Scrivere esplicitamente il 4-tensore impulso-energia per un’onda elettromagnetica piana monocromatica che si propaga lungo l’asse x con densita’ di energia $u_{\text{em}}$.}
\label{sec:3.b.12}
...
\subsection[\hspace{1mm} Ricavare i potenziali di Lienard-Wiechert]{Ricavare l’espressione per i potenziali di Lienard-Wiechert ( $\phi$ ed A per una carica puntiforme in moto arbitrario) a partire dai potenziali ritardati.}
\label{sec:3.b.13}
...
\subsection[\hspace{1mm} Potenza irraggiata da carica in moto non relativistico]{Calcolare la potenza totale irraggiata da una carica accelerata in moto non relativistico. Esprimere i risultati in MKSA e nelle unità “naturali”.}
\label{sec:3.b.14}
...
\subsection[\hspace{1mm} Ricavare la formula di Larmor relativistica a partice da quella non relativistica]{Ricavare la formula di Larmor relativistica 
\[
...
\label{sec:3.b.15}
L'equazione non relativistica è quella ricavata nella domanda precedente.\\
...
\subsection[\hspace{1mm} Frequenza critica della luce di Sincrotrone]{Dimostrare che la radiazione di sincrotrone ha uno spettro di emissione con una frequenza “critica” $\omega_{u}\approx \omega_0\gamma^3$.}
\label{sec:3.b.16}
...
\subsection[\hspace{1mm} Fattore di forma elettromagnetoco per sfera uniformemente carica]{Calcolare il fattore di forma elettromagnetico per una sfera uniformente carica di raggio a.}
\label{sec:3.b.17}
...
\begin{align*}
	...
	\end{cases} 
.\end{align*}
Per un dato valore di $q$ scegliamo delle coordinate sferiche aventi l'asse polare diretto lungo $q$ stesso, si calcola quindi il fattore di forma come:
\begin{align*}
...
\subsection[\hspace{1mm} Fattore di forma per superficie sferica uniformemente carica]{Calcolare il fattore di forma per una supeficie sferica uniformente carica di raggio a. [nota: l'interno è vuoto]}
\label{sec:3.b.18}
...
\subsection[\hspace{1mm} Velocotà di un elettrone in diversi acceleratori lineari]{Calcolare la velocità di un elettrone [e successivamente di un protone] posto in una struttura acceleratrice che abbia: \\
...
.\] 
ci si accorge che il risultato non è tanto differente da quello incontrato nel caso di campo costante (ipotizziamo sempre che la particella sia idealmente ferma nel momento dell'accensione del campo).
\[
...
\subsection[\hspace{1mm} Potenza irraggiata da carica in moto rettilineo]{A partire dai campi ritardati, dimostrare che 
\[
...
\subsection[\hspace{1mm} Energia persa in una rivoluzione per protoni ed elettroni nell'acceleratore circolare]{Calcolare l’energia persa in una rivoluzione per una carica in moto uniforme su una circonferenza (acceleratore circolare). Calcolare la frazione di energia persa
...
\subsection[\hspace{1mm} Potenza angolare emessa per una carica oscillante in linea retta]{Calcolare la potenza emessa in funzione dell’angolo per una carica oscillante armonicamente in linea retta (termine di dipolo elettrico)}
\label{sec:3.b.22}
...
.\] 
Il calcolo da effettuare per ottenere $\hat{f}$ è dato dalla Potenza irraggiata diviso la potenza totale necessaria a portare la particella ad una energia $E_{f}$ all'interno di un acceleratore lineare:
\[
...
\subsection[\hspace{1mm} Lunghezza d'onda critica di sincrotrone in alcuni casi]{Calcolare la lunghezza d’onda critica della radiazione di sincrotrone (elettroni) nei casi seguenti: \\
...
\subsection[\hspace{1mm} Enunciato del teorema ottico]{Enunciare il teorema ottico e spiegarne il significato fisico nel caso di radiazione elettromagnetica su un ostacolo opaco.}
\label{sec:3.b.25}
...
Alcune volte il teorema si trova enunciato anche utilizzando la sezione d'urto totale, il legame tra gli enunciati è ovviamente modulo mediato del vettore di Poynting che va a dividere la $P_{\text{tot}}$ pe ottenere $\sigma_{\text{tot}}$.

\subsection[\hspace{1mm} Sezione d'urto differenziale Rayleigh a partire dalla Thompson]{Come si ricava la sezione d'urto differenziale Rayleigh a partire dalla sezione d'urto Thomson ?}
\label{sec:3.b.26}
